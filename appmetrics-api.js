...
function API(agent, appmetrics) {
  ...
  var raiseEvent = function(topic, message) {
    ...
    switch (topic) {
      ...
      case 'loop_node':
        ...
        break;
      default:
        // Just raise any unknown message as an event so someone can parse it
        // themselves
  ...
  var formatCPU = function(message) {
    ...
    var values = message.trim().split('@#'); // needs to be trimmed because of
    // leading \n character
    var cpu = {
  ...
  var formatOSEnv = function(message) {
    /*
     * environment_os : #EnvironmentSource environment.LESSOPEN=|
     * /usr/bin/lesspipe %s environment.GNOME_KEYRING_PID=1111
     * environment.USER=exampleuser os.arch=x86_64 os.name=Linux
     * os.version=3.5.0-54-generic#81~precise1-Ubuntu SMP Tue Jul 15 04:02:22
  ...
  var formatMemory =
      function(message) {
        ...
        var physicalUsed =
            (physicalTotal >= 0 && physicalFree >= 0)
                ? (physicalTotal - physicalFree) : -1;
        var memory = {
          time: parseInt(values[1]),
