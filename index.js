...
/*
 * Load module probes into probes array by searching the probes directory. We
 * handle the 'trace' probe as a special case because we don't want to put the
 * probe hooks in by default due to the performance cost.
 */
var probes = [];
var traceProbe;
...
/*
 * Patch the module require function to run the probe attach function for any
 * matching module. This loads the monitoring probes into the modules
 */
var data = {};
...
/*
 * Provide API to enable data collection for a given data type. Profiling is
 * done via a control message to the core monitoring agent. Requests require
 * asking all probes to enable request events Other requests are passed to any
 * probe matching the name
 */
module.exports.enable =
    function(data, config) {
      ...
      if (config)
        module.exports.setConfig(data, config);
    };

/*
 * Provide API to disable data collection for a given data type. Profiling is
 * done via a control message to the core monitoring agent. Requests require
 * asking all probes to disable request events Other requests are passed to any
 * probe matching the name
 */
 ...
 * probes except in the case of 'requests'. Here we check for any excludeModules
 * config, and if present use that to control the relevant probes directly.
 */
...
module.exports.transactionLink = function(linkName, callback) {
  if (!this.strongTracerInstrument)
    return callback;
