 ...
 *
 * ID's are assigned to each event. Its important that the count of events is
 * linear for requests as the ID can be used for sampling. It doesn't matter
 * what the event ID is as long as its unique inside a request. We therefore
 * assign: 1) An ID that is the request count or 2) An ID that is the event
 * count
 */
var nextRequestId = 1;
var nextEventId = 1;
...
function Request(type, name, root) {
  ...
  if (this.parent) {
    ...
    this.top = this;
    if (this.type) {
      /* If we're the top and we're a type, we get a
      request Id, not an event Id */
...
Request.prototype.traceStart = function() {
  if (!this.tracedStart) {
    ...
    // we may delay tracing parent requests until later time when they exceed
    // threshold
    if (this.parent) {
      ...
      if (this.parent.type) {
        ...
        this.parentId = this.parent.methId;
      }
    }
  }
};

Request.prototype.traceStop =
  function() {
    if (!this.traceStopped) {
      this.traceStopped = true;

      if ((config.minClockStack !== -1)
...
Request.prototype.stop =
  function(context) {
    if (this.active) {
      ...
      if ((config.minClockTrace !== -1)
          && (this.timer.timeDelta >= config.minClockTrace)) {
        this.traceStart(); // delayed start tracing (will call parent tracing
                            // if needed)
        this.traceStop();
      }
      if (process.domain) {
...
function isNotDeepDiveStack(line) {
  return line.indexOf('appmetrics') === -1
      && line.indexOf('lib/aspect.js') === -1;
}
