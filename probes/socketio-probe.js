...
SocketioProbe.prototype.attach =
    function(name, target) {
      var that = this;
      if (name !== 'socket.io')
        return target;
      /*
       * Don't set __ddProbeAttached__ = true as we need to probe and return the
       * constructor each time
       */

      /*
       * Patch the constructor so that we can patch io.sockets.emit() calls to
       * broadcast to clients. This also picks up calls to io.emit() as they map
       * down to io.socket.emit()
       */
      var newtarget =
          aspect.afterConstructor(target, {}, function(
              target, methodName, methodArgs, context, server) {
            var broadcast = 'broadcast';
            aspect.around(server.sockets, 'emit', function(
              ...
              aspect.around(socket, 'emit', function(
                ...
                if (typeof (callbackPosition) !== 'undefined') {
                  aspect.strongTraceTransactionLink(
                      'socket.io: ', methodName, methodArgs[callbackPosition]);
                }
 ...
 *
 * These provide: time: time event started method: the type of socket.io action
 * event: the event broadcast/emitted/received duration: the time for the action
 * to complete
 */
 ...
 */
SocketioProbe.prototype.requestStart =
    function(context, methodName, methodArgs) {
      ...
      if (methodName !== 'receive') {
        context.req =
            request.startRequest('socketio', methodName, false, context.timer);
      } else {
        context.req =
            request.startRequest('socketio', methodName, true, context.timer);
      }
    };
