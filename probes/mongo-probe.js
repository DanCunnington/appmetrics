...
MongoProbe.prototype.aspectCollectionMethod =
  function(coll, method) {
    var that = this;
    aspect.around(coll, method, function(
        ...
        aspect.aroundCallback(methodArgs, probeData, function(
          ...
          if (typeof (callbackPosition) !== 'undefined') {
            aspect.strongTraceTransactionLink(
                'mongodb: ', methodName, methodArgs[callbackPosition]);
          }
...
MongoProbe.prototype.attach =
    function(name, target) {
      ...
      var method = 'find';
      aspect.around(coll, 'find', function(
          target, methodName, methodArgs, probeData) {
        that.metricsProbeStart(probeData, target, method, methodArgs);
        that.requestProbeStart(probeData, target, method, methodArgs);
 ...
 *
 * These provide: time: time event started query: the query itself duration: the
 ...
 */
MongoProbe.prototype.requestStart =
    function(probeData, target, method, methodArgs) {
      probeData.req =
          request.startRequest(
              'DB', method + '(' + target.collectionName + ')', false,
              probeData.timer);
    };
