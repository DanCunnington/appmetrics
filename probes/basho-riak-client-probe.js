...
var optionsAndCallbackMethods =
    ['deleteValue', 'deleteIndex', 'fetchBucketProps',
    'fetchBucketTypeProps', 'fetchCounter', 'fetchIndex', 'fetchMap',
    'fetchPreflist', 'fetchSchema', 'fetchSet', 'fetchValue',
    'listBuckets', 'listKeys', 'search', 'secondaryIndexQuery',
    'storeBucketProps', 'storeBucketTypeProps', 'storeIndex',
    'storeSchema', 'storeValue', 'tsDelete', 'tsDescribe', 'tsGet',
    'tsListKeys', 'tsQuery', 'tsStore', 'updateCounter', 'updateMap',
    'updateSet' ];
...
RiakProbe.prototype.attach =
    function(name, target) {
      var that = this;
      if (name !== 'basho-riak-client')
        return target;
      ...
      aspect.after(target, 'Client', data, function(
          clientTarget, methodName, args, probeData, rc) {

        // For all methods
        var methods =
            callbackOnlyMethods.concat(
                optionsAndCallbackMethods, commandMethods, queryMethods,
                noParameterMethods);

        // Start probing before the method is executed
        aspect.around(clientTarget, methods, function(
            target, methodName, methodArgs, probeData) {
          ...
          // If the method contains a callback, finish probing when the callback
          // returns
          if (aspect.findCallbackArg(methodArgs) !== undefined) {
            aspect.aroundCallback(methodArgs, probeData, function(
                target, args, probeData) {
        ...
        // Otherwise if there is no callback finish probing when the method
        // returns
 ...
 *
 * These provide: time: Time event started method: The name of the riak method
 * used options: The options parameter (if present) command: The command
 * parameter (if present) query: The query parameter (if present) duration: The
 * time for the request to respond
 */
