      ...
      aspect.aroundCallback(methodArgs, probeData, function(
        ...
        if (typeof (callbackPosition) !== 'undefined') {
          aspect.strongTraceTransactionLink(
              'loopback-datasource-juggler: ', methodName,
...
loopbackDJProbe.prototype.attach =
  function(name, target) {

    var that = this;
    if (name !== 'loopback-datasource-juggler')
      return target;
    ...
    var commands =
      ['create', 'findOrCreate', 'exists', 'find', 'findById', 'remove',
      'removeById', 'count' ];
    var instanceCommands =
        [ 'save', 'remove', 'updateAttribute', 'updateAttributes', 'reload' ];
    var dao = target.Schema.DataAccessObject;

    aspectJugglerMethod(dao, commands, that); // Instrument class methods
    aspectJugglerMethod(dao.prototype, instanceCommands, that); // Instrument
    // instance
    // methods
 ...
 *
 * These provide: time: time event started query: The command the juggler has
 * executed duration: the time for the request to respond
 */
 ...
 */
loopbackDJProbe.prototype.requestStart =
    function(probeData, target, method, methodArgs) {
      var req = request.startRequest('DB', 'query');
      req.setContext({
        loopbackDJProbe: methodArgs[0]
      });
    };
