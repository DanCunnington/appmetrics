 ...
 *
 * Events are fired for calls to the basic commands either in upper or lower
 * case but are always reported as lower case to keep the two sets of events
 * consistent.
 *
 * Calls to batch.exec() and multi.exec() are also instrumented. Their events
 * are batch.exec and multi.exec to ensure they can be separately tracked.
 */
...
RedisProbe.prototype.attach =
    function(name, target) {
      var that = this;
      if (name !== 'redis')
        return;
      if (target.__probeAttached__)
        return target;
      target.__probeAttached__ = true;
      var methods = [];
      [
        'APPEND', 'AUTH', 'BGREWRITEAOF', 'BGSAVE', 'BITCOUNT', 'BITOP',
        'BITPOS', 'BLPOP', 'BRPOP', 'BRPOPLPUSH', 'CLIENT', 'CLUSTER',
        'COMMAND', 'CONFIG', 'DBSIZE', 'DEBUG', 'DECR', 'DECRBY', 'DEL',
        'DISCARD', 'DUMP', 'ECHO', 'EVAL', 'EVALSHA', 'EXISTS', 'EXPIRE',
        'EXPIREAT', 'FLUSHALL', 'FLUSHDB', 'GEOADD', 'GEOHASH', 'GEOPOS',
        'GEODIST', 'GEORADIUS', 'GEORADIUSBYMEMBER', 'GET', 'GETBIT',
        'GETRANGE', 'GETSET', 'HDEL', 'HEXISTS', 'HGET', 'HGETALL',
        'HINCRBY', 'HINCRBYFLOAT', 'HKEYS', 'HLEN', 'HMGET', 'HMSET', 'HSET',
        'HSETNX', 'HSTRLEN', 'HVALS', 'INCR', 'INCRBY', 'INCRBYFLOAT',
        'INFO', 'KEYS', 'LASTSAVE', 'LINDEX', 'LINSERT', 'LLEN', 'LPOP',
        'LPUSH', 'LPUSHX', 'LRANGE', 'LREM', 'LSET', 'LTRIM', 'MGET',
        'MIGRATE', 'MONITOR', 'MOVE', 'MSET', 'MSETNX', 'OBJECT', 'PERSIST',
        'PEXPIRE', 'PEXPIREAT', 'PFADD', 'PFCOUNT', 'PFMERGE', 'PING',
        'PSETEX', 'PSUBSCRIBE', 'PUBSUB', 'PTTL', 'PUBLISH', 'PUNSUBSCRIBE',
        'QUIT', 'RANDOMKEY', 'RENAME', 'RENAMENX', 'RESTORE', 'ROLE', 'RPOP',
        'RPOPLPUSH', 'RPUSH', 'RPUSHX', 'SADD', 'SAVE', 'SCARD', 'SCRIPT',
        ...
        'ZREVRANGEBYLEX', 'ZRANGEBYSCORE', 'ZRANK', 'ZREM', 'ZREMRANGEBYLEX',
        'ZREMRANGEBYRANK', 'ZREMRANGEBYSCORE', 'ZREVRANGE',
        'ZREVRANGEBYSCORE', 'ZREVRANK', 'ZSCORE', 'ZUNIONSTORE', 'SCAN',
        'SSCAN', 'HSCAN', 'ZSCAN'
      ].map(function(m) {
        methods.push(m);
      ...
      aspect.around(target.RedisClient.prototype, methods, function(
          target, method, methodArgs, probeData) {
        ...
        /*
         * REDIS commands don't have to have a callback. All redis calls are
         * asynchronous so we need to instrument or add a callback to stop the
         * timer.
         */
        aspect.aroundCallback(methodArgs, probeData, function(target, args) {

          // Call the transaction link with a name and the callback for strong
          // trace
          var callbackPosition = aspect.findCallbackArg(methodArgs);
          aspect.strongTraceTransactionLink(
              'redis: ', eventName, methodArgs[callbackPosition]);
       ...
       * or client.multi()
       */
      aspect.after(target.RedisClient.prototype, [
        'multi', 'batch', 'MULTI', 'BATCH' ], {}, function(
          target, mode, args, probeData, client) {
          // Log the event name as batch.exec or multi.exec
          var eventName = mode.toLowerCase() + '.exec';
          aspect.around(client, [ 'exec', 'EXEC' ], function(
              target, method, methodArgs, probeData) {
            that.metricsProbeStart(probeData, eventName, methodArgs);
            that.requestProbeStart(probeData, eventName, methodArgs);
            /*
             * REDIS commands don't have to have a callback. All redis
             * calls are asynchronous so we need to instrument or add a
 ...
 *
 * These provide: time: time event started cmd: REDIS method, eg. GET, SET,
 * INCR, etc duration: the time for the request to respond
 */
