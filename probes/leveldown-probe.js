...
function aspectLvldownMethod(dbTarget, methods, probe) {
  aspect.before(dbTarget, methods, function(
      dbTarget, methodName, methodArgs, probeData) {
      ...
      aspect.aroundCallback(methodArgs, probeData, function(
        ...
        if (typeof (callbackPosition) !== 'undefined') {
          aspect.strongTraceTransactionLink(
              'leveldown: ', methodName, methodArgs[callbackPosition]);
        }
 ...
 *
 * These provide: time: time event started method: leveldown method being
 * executed key: The key being used for a call to `get`, `put` or `del` value:
 * The value being added to the LevelDB database using `put` opCount: The number
 * of operations being performed by `batch` duration: The time taken for the
 * LevelDB query to respond in ms
 *
 * Note: key, value and opCount are undefined for methods other than those *
 * stated
 */
 ...
 */
LeveldownProbe.prototype.requestStart =
    function(probeData, dbTarget, method, methodArgs) {
      var req = request.startRequest('DB', 'query');
      req.setContext({
        leveldown: methodArgs[0]
      });
    };
