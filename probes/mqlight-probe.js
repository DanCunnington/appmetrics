...
MQLightProbe.prototype.attach =
    function(name, target) {
      ...
      aspect.after(target, 'createClient', {}, function(
          target, methodName, createClientArgs, context, rc) {
        ...
        aspect.around(thisClient, methods, function(
            target, methodName, args, probeData) {
          ...
          // Advise the callback for the method. Will do nothing if no callback
          // is
          // registered
          aspect.aroundCallback(args, probeData, function(
              target, callbackArgs, probeData) {
            // method has completed and the callback has been called, so end the
            // monitoring
            ...
            if (typeof (callbackPosition) !== 'undefined') {
              aspect.strongTraceTransactionLink(
                  'mqlight: ', methodName, args[callbackPosition]);
            }
          ...
          // If no callback used then end the monitoring after returning from
          // the
          // method instead
        ...
        aspect.before(thisClient, 'on', function(
          ...
          if (args[0] === 'message') {
            ...
            aspect.aroundCallback(args, {}, function(
              ...
              // method has completed and the callback has been called, so end
              // the
              // monitoring
              ...
              if (typeof (callbackPosition) !== 'undefined') {
                aspect.strongTraceTransactionLink(
                    'mqlight: ', methodName, callbackArgs[callbackPosition]);
              }

              that.metricsProbeEnd(
                  probeData, 'message', callbackArgs, thisClient);
 ...
 */
MQLightProbe.prototype.metricsEnd =
    function(probeData, method, methodArgs, client) {
 ...
 */
MQLightProbe.prototype.requestStart =
    function(probeData, method, methodArgs) {
      if (method === 'message') {
        probeData.req =
            request.startRequest('MQLight', method, true, probeData.timer);
      } else {
        probeData.req =
            request.startRequest('MQLight', method, false, probeData.timer);
      }
    };

MQLightProbe.prototype.requestEnd =
    function(probeData, method, methodArgs, client) {
      var data;
      if (method === 'message') {
        data = methodArgs[0];
