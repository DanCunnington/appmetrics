...
Strong_MQProbe.prototype.attach =
    function(name, target) {
      ...
      aspect.after(target, 'create', {}, function(
          target, methodName, methodArgs, context, connection) {

        aspect.after(
            connection, [ 'createPushQueue', 'createPubQueue' ], {}, function(
                target, methodName, methodArgs, context, queue) {
              var socketType = methodName;
              var methods = typeToMethods[socketType];
              aspect.around(queue, methods, function(
                  target, methodName, methodArgs, context) {
        ...
        aspect.after(
            connection, [ 'createPullQueue', 'createSubQueue' ], {}, function(
                target, methodName, methodArgs, context, queue) {
              var socketType = methodName;
              var methods = typeToMethods[socketType];
              aspect.before(queue, methods, function(
                ...
                if (aspect.findCallbackArg(methodArgs) !== undefined) {
                  aspect.aroundCallback(methodArgs, context, function(
                    ...
                    // Call the transaction link with a name and the callback
                    // for
                    // strong trace
                    var callbackPosition = aspect.findCallbackArg(methodArgs);
                    if (typeof (callbackPosition) !== 'undefined') {
...
/*
 * Lightweight metrics probe for STRONG-MQ messaging Provide basic information
 * on messages sent and received. These provide: time: time event started type:
 * whether this was a pub, sub, push or pull event. method: if this was a method
 * call (a send) the method name event: if this was an event (a message was
 * received) the name of the event duration: the time for the request to respond
 */
Strong_MQProbe.prototype.metricsEnd =
    function(context, methodName, methodArgs, eventType) {
      context.timer.stop();
      // default to quality of service (qos) 0, as that's what the strong-mq
      // module
      // does
      var eventData = {
 ...
 */
Strong_MQProbe.prototype.requestStart =
    function(context, methodName, methodArgs, socketType) {
      if (methodName === 'publish') {
        context.req =
            request.startRequest('STRONG-MQ', methodName, false, context.timer);
      } else {
        /* Received messages mark the start of requests. */
        context.req =
            request.startRequest('STRONG-MQ', 'message', true, context.timer);
      }
    };

Strong_MQProbe.prototype.requestEnd =
    function(context, methodName, methodArgs, socketType) {
      context.req.stop({
        topic: methodArgs[0]
      });
