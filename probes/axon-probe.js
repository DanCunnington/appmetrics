...
AxonProbe.prototype.attach =
    function(name, target) {
      ...
      aspect.after(target, [ 'socket' ], {}, function(
          target, methodName, methodArgs, context, client) {
        var socketType = methodArgs[0];
        var methods;
        if (isSendMethod(socketType)) {
          methods = sendTypeToMethod[socketType];
          aspect.around(
              client, methods,
                ...
                aspect.aroundCallback(methodArgs, context, function(
                  ...
                  if (typeof (callbackPosition) !== 'undefined') {
                    aspect.strongTraceTransactionLink(
                        'axon', methodName, methodArgs[callbackPosition]);
                  }

                  that.metricsProbeEnd(
                      context, methodName, methodArgs, socketType);
            ...
            if (aspect.findCallbackArg(methodArgs) !== undefined) {
              aspect.aroundCallback(
                  methodArgs, context, function(target, args, context) {
                    ...
                    if (typeof (callbackPosition) !== 'undefined') {
                      aspect.strongTraceTransactionLink(
                          'axon: ', eventName, args[callbackPosition]);
                    }

                    that.metricsProbeStart(
                        context, eventName, methodArgs, socketType);
...
/*
 * Lightweight metrics probe for AXON messaging Provide basic information on
 * messages sent and received. These provide: time: time event started method:
 * whether this was a received message 'in' or sent 'out' type: topic: the topic
 * the message was received on duration: the time for the request to respond
 */
AxonProbe.prototype.metricsEnd =
    function(context, methodName, methodArgs, socketType) {
      context.timer.stop();
      // default to quality of service (qos) 0, as that's what the axon module
      // does
      if (isSendMethod(socketType)) {
        am.emit('axon', {
 ...
 */
AxonProbe.prototype.requestStart =
    function(context, methodName, methodArgs) {
      if (methodName === 'message') {
        context.req =
            request.startRequest('AXON', methodName, true, context.timer);
      } else {
        context.req =
            request.startRequest('AXON', methodName, false, context.timer);
      }
    };
