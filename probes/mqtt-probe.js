...
MqttProbe.prototype.attach =
    function(name, target) {
      ...
      aspect.after(target, 'connect', {}, function(
        ...
        var methods = 'publish';
        aspect.around(client, methods, function(
          ...
          aspect.aroundCallback(methodArgs, context, function(
            ...
            if (typeof (callbackPosition) !== 'undefined') {
              aspect.strongTraceTransactionLink(
                  'mqtt: ', methodName, methodArgs[callbackPosition]);
            }
 ...
 *
 * These provide: time: time event started method: whether this was a received
 * 'message' or a 'publish' topic: the topic the message was received on qos:
 * the quality of service (QoS) for the message duration: the time for the
 * request to respond
 */
 ...
 */
MqttProbe.prototype.requestStart =
    function(context, methodName, methodArgs) {
      if (methodName === 'message') {
        context.req =
            request.startRequest('MQTT', methodName, true, context.timer);
      } else {
        context.req =
            request.startRequest('MQTT', methodName, false, context.timer);
      }
    };
