...
TraceProbe.prototype.attach =
    function(moduleName, target) {
      if (moduleName.slice(0, 1) !== '.'
          || stopList[moduleName] || !isAppInnerRequire()
...
function instrument(target, name, method, fullName) {
  ...
  var methodargs =
      methodString.toString().split(')')[0].split('(')[1].split(',');
  var lastMethodArg = methodargs[methodargs.length - 1].replace(/ /g, '');
  if (lastMethodArg === '')
    lastMethodArg = 'undefined';
      ...
      function() {
            ...
            function() {
              ...
              for (var i = 0; i < args.length; ++i) {
                ...
                if (typeof (arg) === 'function'
                    && Object.keys(arg).length === 0 && arg.prototype
        ...
        if (arguments.length > 0
            ...
            && (!arguments[arguments.length - 1].hasOwnProperty('prototype') ||
                (arguments[arguments.length - 1].prototype && Object
                .keys(arguments[arguments.length - 1].prototype).length === 0))
            && methodString.indexOf(lastMethodArg + '.call') < 0
            && methodString.indexOf(lastMethodArg + '.apply') < 0
            && ('' + arguments[arguments.length - 1])
                .indexOf('instrumentedMethodKNJ') < 0) {
          isCallback = true;
...
function isResponseMethod(args) {
  if (args.length > 1) {
    ...
    if (typeof (arg) === 'object'
        && arg !== null && 'send' in arg && 'render' in arg
...
function isAppInnerRequire() {
  ...
  var callerLine = trace.stack.split('\n'); // This line contains 'node_modules'
  // reference for generic libs
  return callerLine[6].indexOf('node_modules') === -1;
}

function instrumentMethods(moduleName, target) {
  for (var name in target) {
    if (!target.__lookupGetter__(name)
        && typeof (target[name]) === 'function') {
      if (!target[name].__super__
          && (target[name].prototype || (target[name].prototype && Object
              .keys(target[name].prototype).length === 0))
