...
StrongOracleProbe.prototype.attach =
    function(name, target) {
      var that = this;
      if (name !== 'strong-oracle')
        return target;
      ...
      var newtarget =
          aspect.afterConstructor(target, {}, function(
              target, methodName, methodArgs, context, ret) {
            ...
            aspect.before(ret, 'connect', function(
              ...
              aspect.aroundCallback(args, {}, function(
                ...
                if (!err) {
                  ...
                  // Don't attach to the same connection more than once when
...
function addMonitoring(connection, probe) {
  aspect.around(connection, 'execute', function(
      target, methodName, args, probeData) {
    ...
    // Advise the callback for 'execute'. Will do nothing if no callback is
    // registered
    aspect.aroundCallback(args, probeData, function(
        target, callbackArgs, probeData) {
      // 'execute' has completed and the callback has been called, so end the
      // monitoring
      // Call the transaction link with a name and the callback for strong trace
      var callbackPosition = aspect.findCallbackArg(args);
      if (typeof (callbackPosition) !== 'undefined') {
        aspect.strongTraceTransactionLink(
            'strong-oracle: ', methodName, args[callbackPosition]);
      }
    ...
    // If no callback used then end the monitoring after returning from the
 ...
 */
StrongOracleProbe.prototype.metricsEnd =
 ...
 */
StrongOracleProbe.prototype.requestStart =
    function(probeData, method, methodArgs) {
      probeData.req =
          request.startRequest('StrongOracle', method, false, probeData.timer);
    };

StrongOracleProbe.prototype.requestEnd =
    function(probeData, method, methodArgs) {
      var query = methodArgs[0];
      probeData.req.stop({
        query: query
